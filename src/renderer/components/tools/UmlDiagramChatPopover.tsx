...
export function UmlDiagramChatPopover() {
  const {
    activeFile,
    isChatOpen,
    activeFileChat,
    ...
    <div className="pointer-events-none fixed inset-0 z-[48]">
      {isChatOpen ? (
        <button
          ...
          onClick={closeChat}
          tabIndex={-1}
          aria-label="Fechar chat do diagrama"
        />
      ) : null}
      <div className="pointer-events-auto fixed bottom-6 right-6">
        <Button
          ...
          aria-label={isChatOpen ? "Fechar chat do diagrama" : "Abrir chat do diagrama"}
          title={isChatOpen ? "Fechar chat do diagrama" : "Abrir chat do diagrama"}
        >
          {isChatOpen ? (
          ...
          <div className="absolute bottom-14 right-0 flex h-[80dvh] min-w-[40dvw] w-[26rem] flex-col overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-2xl dark:border-zinc-700 dark:bg-zinc-900">
            <div className="flex items-start justify-between gap-3 border-b border-zinc-200 px-3 py-2 dark:border-zinc-700">
              <div className="min-w-0">
                <p className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Chat UML</p>
              ...
              <Button
                ...
                aria-label="Limpar conversa deste diagrama"
              ...
              {activeFileChat.length > 0 ? (
                ...
                !isSending && (
                  <p className="text-xs text-zinc-500 dark:text-zinc-400">
                    {activeFile
                      ? "Peça alterações no diagrama PlantUML. As sugestões abrem no editor para revisão."
                      : "Abra ou crie um arquivo .puml para usar o chat."}
                  </p>
                )
              )}
            ...
            <div className="border-t border-zinc-200 px-3 py-2 dark:border-zinc-700">
              <p className="mb-2 text-[11px] text-zinc-500 dark:text-zinc-400">
                Alterações não são aplicadas automaticamente — revise no editor.
              </p>
              <ChatAiControls
